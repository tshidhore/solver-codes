...
def Divergence_facv(part_names, xy_no, xy_fa, xy_cv, noofa, cvofa, faono, faocv, partofa):
    ...
    partofa1 = np.array(partofa) # Converting partofa to an array
    faono1 = np.array(faono) # Converting faono to an array
    lid_bc = np.where(partofa1=='LID')   # Vectorized approach to find face belonging to part 'COLD'
    wall_bc = np.where(partofa1=='WALL')   # Vectorized approach to find face belonging to part 'HOT'
    ...
    for j in np.arange(nno):
        ...
        wall_count = np.array(np.where(part_name == 'WALL')).size
        lid_count = np.array(np.where(part_name == 'LID')).size
        if wall_count != 0:
            partono.append('WALL')
        elif lid_count != 0:
            partono.append('LID')
...
def gradient_nn(part_names, xy_no, xy_fa, xy_cv, noofa, cvofa, faono, faocv, partofa):
    
    nno = xy_no.shape[0]  # No. of nodes
    ncv = xy_cv.shape[0]  # No. of CVs
    partofa1 = np.array(partofa) # Converting partofa to an array
    faono1 = np.array(faono) # Converting faono to an array
    wall_bc = np.where(partofa1=='WALL')   # Vectorized approach to find face belonging to part 'COLD'
    lid_bc = np.where(partofa1=='LID')   # Vectorized approach to find face belonging to part 'HOT'
    solid = np.where(partofa1=='SOLID')   # Vectorized approach to find face belonging to part 'SOLID'
    ...
    nno_int = nno-np.unique(noofa[wall_bc]).size-np.unique(noofa[lid_bc]).size # No. of internal nodes
...
def Avg_fan_int(part_names, xy_no, xy_fa, xy_cv, noofa, cvofa, faono, faocv, partofa):
    ...
    nno_int = nno-np.unique(noofa[wall]).size-np.unique(noofa[lid]).size # No. of internal nodes
